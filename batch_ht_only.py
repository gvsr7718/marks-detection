...
out_file = "htno_list.md"

with open(out_file, "w", encoding="utf-8") as f:
    f.write("| S.No | Image Name | Extracted HT No |\n")
    f.write("|---|---|---|\n")
    ...
    for idx, path in enumerate(image_paths):
        ...
        try:
            ...
            # Format to ensure max 10 characters
            if len(ht_no) > 10:
                ht_no = ht_no[:10]
            elif ht_no == "NOT_FOUND":
                pass
            
            f.write(f"| {idx + 1} | {basename} | {ht_no} |\n")
            f.flush()
        except Exception as e:
            print(f"Error processing {basename}: {e}")
            f.write(f"| {idx + 1} | {basename} | ERROR |\n")

print("HTNO extraction complete! Results saved to htno_list.md")
