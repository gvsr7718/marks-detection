...
def _pick_marks_table(gray, img_color, tables, search_h):
    ...
    non_ht = [t for t in tables if not (len(t['rows']) == 1 and _looks_like_ht_row(t))]
    if non_ht:
        return max(non_ht, key=lambda t: len(t['cells']))
    if tables:
        return max(tables, key=lambda t: len(t['cells']))
    return None
...
def _extract_bottom_row_cells(gray, table):
    """Extract crops from the MARKS (bottom) row of the table."""
    rows = table.get('rows', [])
    if not rows:
        return [], None, table
    ...
    for i, c in enumerate(marks_row_sorted):
        ...
        # Skip label cell ('MARKS' or 'Q.NO.' text cell)
        # It's typically much wider than digit cells
        if i == 0 and len(marks_row_sorted) > 2:
            median_w = sorted([c2['w'] for c2 in marks_row_sorted])[len(marks_row_sorted)//2]
            if c['w'] > median_w * 1.5:
                continue
...
def extract_ht_number_boxes(gray: np.ndarray, thresh: np.ndarray) -> tuple:
    ...
    # The "H.T. No." label is typically the first cell and very wide (>120px).
    # We must exclude it so it doesn't get passed to OCR.
    start_idx = 0
    ...
    # Add horizontal padding to prevent OCR from clipping the first/last characters
    x_min = max(0, x_min - 15)
    y_min = max(0, y_min - 2)
    x_max = min(width, x_max + 15)
    ...
    # Individual box crops for debug
    boxes = []
    for c in box_cells:
        x, y, w, h = c['x'], c['y'], c['w'], c['h']
        pad_x = int(w * 0.15)
        pad_y = int(h * 0.15)
        crop = gray[max(0, y + pad_y):min(height, y + h - pad_y),
